...
describe('PATCH /todos/:id', () => {
	...
	it('should NOT update a todo created by another user', (done) => {
		...
		var updatedText = 'Text now updated via testing module';
		var oldText;
		Todo.findById(hexId, function(err, todo) {
			if (!err) {
				oldText = todo.text;
			}
		});
		...
		request(app)
			...
			.end( (err, res) => {
				...
				User.findById(users[1]._id).then( (user) => {
					expect(user.tokens[1]).toInclude({	// tokens[1] because there already exist a token in tokens[0] added while seeding users
						access: 'auth',
	...
	it('should reject invalid login', (done) => {
		request(app)
			...
			.end( (err, res) => {
				...
				User.findById(users[1]._id).then( (user) => {
					expect(user.tokens.length).toBe(1);	// already has one token added in seed.js so check that there aren't any more
					done();
				} ).catch( (e) => done(e) );
			} );
	});
